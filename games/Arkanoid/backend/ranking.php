        ...
        if ($id_game !== -1 && $id_user !== -1 && $points !== -1) {
            ...
            if ($statement->rowCount() > 0) {
                ...
                if ($current_ranking_user && ($current_ranking_user["points"] < $points)) {
                    $statement = $db->connect()->prepare("UPDATE user_game_cycle SET points = :points WHERE id_user = :id_user AND id_game = :id_game");
                ...
                $statement = $db->connect()->prepare("INSERT INTO user_game_cycle (id_user, id_game, points) VALUES (:id_user, :id_game, :points)");
